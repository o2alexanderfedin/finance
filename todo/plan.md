...
3. **The restricted runner.** A **total** `ToAsyncOperationMap` over that vocabulary, driven by `asyncRun` — total over a narrow set, not a wide set with entries removed. Guard dispatch with `Object.hasOwn` (a stored blob can emit any `command` string whatever its declared type) and report a miss as `operation not permitted: <command>`. See [`fjs/todo/restricted-runner-operation-map.md`](../fjs/todo/restricted-runner-operation-map.md).
...
4. ~~**Deadline and definition of done.**~~ **Answered** in [issue #16](https://github.com/fjs-dev/finance/issues/16#issuecomment-5171851184): the deadline is **five weeks**, externally driven. Minimal definition of done — users add raw documents to CAS through their AI agent, then ask for reports; when CAS already holds a report for that year and user, the agent **updates** it as a new Evo revision rather than writing a fresh one; each tax year may carry different rules and forms. Scope is deliberately a small subset of reports and documents, narrowed during development rather than fixed up front, with a running list of what is covered and what still needs implementing. A dynamic system where the AI adds new document types is the eventual aim but **out of scope for these five weeks**. See the Settled Decisions table above, and question 7 for the one part of this that is not yet settled.
...
7. **Single user, or multiple?** *New, and it contradicts a Settled Decision above.* The definition of done in [issue #16](https://github.com/fjs-dev/finance/issues/16#issuecomment-5171851184) says "**multiple users** can add their raw documents to CAS", and "if CAS already has a report for that year **and user**" — which reads as one store holding several users' data. Every current decision assumes the opposite: *Transport / audience* is "stdio only, single local user", and *Import-time safety* accepts unrestricted `import()` as "tolerable **only** because the sole user is trusted and local". The spec is explicit that this is the hinge: "if the audience ever widens beyond one local user, it is a blocker, not a cleanup task."

   Two readings, and they differ enormously in cost:
   - **Several users, each with their own personal CAS and their own stdio server.** Nothing changes — "personal CAS" already implies one per person, and "that year and user" is just a subject-naming detail feeding question 2. Cheap.
   - **One deployment serving several users.** Then per-user isolation, authentication, and a real answer to import-time execution all move into scope, and the accepted `import()` limitation becomes a v1 blocker rather than a Week 5 revisit.

   **Answer before Phase 1 / Week 1 Track A**, because it decides whether the restricted runner is sufficient or merely a first layer — and, with question 5, what `CasOp` has to express.
